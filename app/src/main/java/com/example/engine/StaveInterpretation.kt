...
object StaveInterpretation {
    ...
    fun generate(runes: List<Rune>, layoutType: StaveLayoutType): StaveInterpretationData {
        ...
        val title = when {
            ...
            layoutType == StaveLayoutType.GALDRABOK_ASYMMETRIC -> "Исландский гальдрастав (${runes.joinToString(" • ") { it.nameRu }})"
        ...
        val summary = buildString {
            ...
            when (layoutType) {
                ...
                StaveLayoutType.SOLAR_12_RAY -> append("12-лучевая солярная розетка концентрирует колоссальную энергию 12 годовых и небесных циклов для всеобъемлющей защиты и процветания.")
                ...
                StaveLayoutType.GALDRABOK_ASYMMETRIC -> append("Асимметричная исландская вязь Гальдрабука с крюками, молниями и солярными очагами запутывает чужеродную ментальную магию и мгновенно отражает любые астральные угрозы.")
            }
        }
        ...
        val strokeOrder = when (layoutType) {
            ...
            StaveLayoutType.SOLAR_12_RAY -> "1. Наносится центральное двоякое кольцо Солнца. 2. Чертится 12 основных лучей посолонь (по часовой стрелке). 3. Вписываются связующие орбиты и руны на лучах."
            StaveLayoutType.GALDRABOK_ASYMMETRIC -> "1. Чертится главная сокрытая ось гальдрастава сверху вниз. 2. Завязываются спиральные крюки и перемычки молний слева направо. 3. Вписываются руны в ответвленные узлы силы."
